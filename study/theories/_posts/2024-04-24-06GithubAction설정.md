...
# Github Action과 aws의 ec2, rds 사용해서 Spring boot 프로젝트 배포하기-Github Action 설정
...
1. 프로젝트 **리포지터리 메인 화면**에서 **Settings 탭**을 클릭한다. 
...
2. **Security -> Secrets and variables -> Actios** 경로로 들어가 Repository secrets 에서 **new repository secrets** 을 클릭해준다.
...
3. **Name**에 값을 빼내올 때 사용할 **키의 이름**, **Scret**에 저장해야할 **값**을 넣고 **Add secret** 하면 된다.
...
4. **aws 지역**, **IAM 사용자 액세스 키**, **application.yml** 을 등록해놨다.
...
5. application.yml 의 경우 전체 내용을 **Base64로 인코딩** 한후 넣어줬는데 [이 사이트](https://www.convertstring.com/ko/EncodeDecode/Base64Encode)를 이용하면 편리하다.
...
1. 프로젝트 **리포지터리 메인화면**에서 **Actions 탭** 클릭
...
2. 우리 프로젝트는 메이븐으로 만들어졌으므로 **Java with Maven 의 Configure** 클릭
...
3. 자동적으로 빌드와 관련된 작업흐름이 적힌 파일이 생성된다. </br>
...
1. codedeploy가 수행할 명령을 작성한 **appspec.yml** 을 생성한다.
...
4. 위 파일들을 scripts 폴더에 같이 넣어놓고 마찬가지로 **리포지터리의 최상단**에 위치시킨다.
